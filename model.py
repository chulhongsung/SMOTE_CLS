...
class MLPEncoder(nn.Module):
    def __init__(self, input_dim, latent_dim, class_num, device='cpu'):
        ...
        self.net = nn.Sequential(
            nn.Linear(self.input_dim, 8 * self.latent_dim * self.class_num),
            nn.ELU(),
            nn.Linear(8 * self.latent_dim * self.class_num, 4 * self.latent_dim * self.class_num),
            nn.ELU(),
            nn.Linear(4 * self.latent_dim * self.class_num, 2 * self.latent_dim * self.class_num),
        ).to(device)
...
class MLPDecoder(nn.Module):
    def __init__(self, input_dim, latent_dim, activation="identity", device='cpu'):
        ...
        self.net = nn.Sequential(
            ...
            nn.ELU(),
            nn.Linear(4 * self.latent_dim, 8 * self.latent_dim),
            nn.ELU(),
            nn.Linear(8 * self.latent_dim, self.input_dim)
        ).to(device)
...
class MLPClassifier(nn.Module):
    def __init__(self, input_dim, latent_dim, activation="softmax", device='cpu'):
        super(MLPDecoder, self).__init__()
        self.input_dim = input_dim
        self.latent_dim = latent_dim
        self.activation = activation
...
class MixtureMLPVAE(nn.Module):
    def __init__(self, config, class_num, classifier, device='cpu'):
        ...
        self.decoder = MLPDecoder(config["input_dim"], config["latent_dim"], device=device)
    ...
    def classify(self, input):
        ...
        if self.classifier.__class__ == xgboost.sklearn.XGBClassifier:
            input_ = input.detach().cpu().numpy()
            probs_ = self.classifier.predict_proba(input_)
            probs = torch.Tensor(probs_)
        else:
...
class Decoder(nn.Module):
    def __init__(self, input_dim, latent_dim, activation="identity", device='cpu'):
        ...
        self.net = nn.Sequential(
            ...
            nn.ELU(),
            nn.Linear(4 * self.latent_dim, self.input_dim)
        ).to(device)
